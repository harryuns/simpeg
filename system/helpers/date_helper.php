...
if ( ! function_exists('ConvertDateToArray'))
{
	function ConvertDateToArray($Date) {
		...
		$Array['Year'] = $ArrayTemp[0];
		$Array['Month'] = $ArrayTemp[1];
		$Array['Day'] = $ArrayTemp[2];
		return $Array;
	}
}

if ( ! function_exists('ConvertDateToArrayUnix'))
{
    function ConvertDateToArrayUnix($DataDate, $YearMonthDay = true, $ReplaceMonth = true) {
		if (empty($DataDate)) {
			return array();
		}
		
		$Array = array();
        $Date = substr($DataDate, 0, 10);
        $ArrayTemp = explode('-', $Date);
        ...
        if ($YearMonthDay) {
            ...
            $Array['Year'] = $ArrayTemp[2];
            $Array['Month'] = $ArrayTemp[1];
            $Array['Day'] = $ArrayTemp[0];
        }
        
        preg_match('/ (\d+):(\d+):(\d+)$/i', $DataDate, $Match);
		$Array['Hour'] = $Array['Minute'] = $Array['Second'] = '00';
        if (isset($Match[1]) && !empty($Match[1])) {
            $Array['Hour'] = (isset($Match[1])) ? $Match[1] : '00';
            $Array['Minute'] = (isset($Match[2])) ? $Match[2] : '00';
            $Array['Second'] = (isset($Match[3])) ? $Match[3] : '00';
        }
        
		if ($ReplaceMonth) {
			$Array['Month'] = preg_replace('/^0/i', '', $Array['Month']);
		}
...
if ( ! function_exists('ConvertDateToArrayExt'))
{
    function ConvertDateToArrayExt($DataDate, $YearMonthDay = false) {
        $Array = array();
        $Date = substr($DataDate, 0, 10);
        $ArrayTemp = explode('-', $Date);
...
{
	function ConvertDateToString($String, $Param = array('DateFormat' => "j F Y")) {
        if (empty($String)) {
            return '';
        }
        
		$ArrayDate = ConvertDateToArray($String);
		if ($Param['DateFormat'] == 'j F Y' && $ArrayDate['Year'] > 2015) {
			$Result = date('j F', mktime (0, 0, 0, $ArrayDate['Month'], $ArrayDate['Day'], date("Y"))) . ' ' . $ArrayDate['Year'];
		} else {
			$Result = date($Param['DateFormat'], mktime (0, 0, 0, $ArrayDate['Month'], $ArrayDate['Day'], $ArrayDate['Year']));
		}
...
if (! function_exists('GetUnixTime')) {
    function GetUnixTime($Date) {
        $ArrayDate = ConvertDateToArrayUnix($Date, true);
