...
if ( ! function_exists('Upload'))
{
    function Upload($File, $Name = 'Image') {
        $ArrayResult = array('Message' => '');
        
        if (isset($_FILES[$Name]) && !empty($_FILES[$Name])) {
            if (isset($File['Extention'])) {
                $FileName = $File['Path'].basename($File['Name']);
                $FileLocation = $FileName.'.'.$File['Extention'];
                @unlink($FileName.'.jpg');
                @unlink($FileName.'.jpeg');
                @unlink($FileName.'.pdf');
            } else {
                $FileLocation = $File['Path'].basename($File['Name']);
                @unlink($FileLocation);
            }
            
            if (move_uploaded_file($_FILES[$Name]['tmp_name'], $FileLocation)) {
