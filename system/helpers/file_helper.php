...
{
    function Upload($File, $Name = 'Image') {
        ...
        if (isset($_FILES[$Name]) && !empty($_FILES[$Name])) {
			...
			$PathFileName = PATH . '/images/_Temp/'. $FileName;
			@unlink($PathFileName);
			...
			if ($ArrayResult['Status'] == 1) {
				...
				if ($File['WithCreateDir'] == 1) {
					...
					foreach ($DirFile as $Directory) {
						$DateDir .= $Directory . '/';
						$File['Path'] .= $Directory . '/';
						@ftp_mkdir($FtpConnection, $File['Path']);
					}
				}
				
				// Delete File with same name
				$FileNameDelete = $File['Path'] . basename($File['Name']);
				foreach (array('jpg', 'png', 'jpeg', 'pdf') as $Value) {
					@ftp_delete($FtpConnection, $FileNameDelete . '.' . $Value);
				}
				
				ftp_chdir($FtpConnection, $File['Path']);
				$FtpDelete = @ftp_delete($FtpConnection, $FileName);
...
if ( ! function_exists('WriteLogErrorQuery'))
{
    function WriteLogErrorQuery($StringQuery) {
        ...
        $StringQuery = preg_replace('/\s+/i', ' ', $StringQuery)."\n";
        $FileName = PATH.'/Log/Query/'.date("Ymd").'.txt';
        Write(PATH.'/Log/Query/'.date("Ymd").'.txt', $StringQuery);
    }
}
