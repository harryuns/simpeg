...
//	$is_user_fakultas = true;
	...
	$array_pegawai_honorer = $this->riwayat_honorer_model->get_array(array( 'K_PEGAWAI' => $k_pegawai ));
	$array_pegawai_honorer_request = $this->riwayat_honorer_request_model->get_array(array( 'K_PEGAWAI' => $k_pegawai, 'IS_VALIDATE' => 0 ));
?>

<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<?php $this->load->view('body_meta', array( 'page_title' => 'Riwayat Penempatan Kerja' ) ); ?>
...
<div id="body"><div id="frame">
	...
	<div id="content"><div class="full" style="min-height: 400px;"><div id="CntRightFull">
		...
		<div class="cnt-grid">
			<?php if (count($array_pegawai_honorer) > 0) { ?>
				<h1>Riwayat Penempatan Kerja</h1>
				<div class="cnt_table_main record-valid"><table style="width: 1200px;">
					<tr>
					...
					<?php foreach ($array_pegawai_honorer as $row) { ?>
					<tr>
						<td class="licon">
			...
			<?php if (count($array_pegawai_honorer_request) > 0) { ?>
				...
				<div class="cnt_table_main record-request"><table style="width: 1350px;">
					<tr>
						<td class="left" style="width: 175px;">&nbsp;</td>
					...
					<tr>
						...
						<td class="body"><?php echo show_jenis_request($row['JENIS_REQ_HONORER']); ?></td>
						<td class="body"><?php echo $row['NO_SK']; ?></td>
						<td class="body"><?php echo ConvertDateToString($row['TGL_SK']); ?></td>
						<td class="body"><?php echo $row['ASAL_SK']; ?></td>
						<td class="body"><?php echo ConvertDateToString($row['TMT']); ?></td>
		...
		<div class="cnt-form hidden">
			...
			<form style="width: 80%;" id="FormRiwayatHomeBase" action="<?php echo base_url('index.php/pegawai_modul/riwayat_honorer/action'); ?>">
				<input type="hidden" name="action" />
				<input type="hidden" name="ID_REQ_HONORER" value="x" />
				<input type="hidden" name="ID_RIWAYAT_HONORER" value="x" />
				<input type="hidden" name="K_PEGAWAI" value="<?php echo $k_pegawai; ?>" />
				<input type="hidden" name="JENIS_REQ_HONORER" />
				...
				<table style="width: 100%;" class="tabel" cellspacing="0" cellpadding="5" border="0">
					...
					<tr class="hidden">
						...
						<td style="width: 300px;"><select style="width: 85%;" name="K_JABATAN_FUNGSIONAL">
							<?php echo GetOption(false, $array_jabatan_fungsional, ''); ?>
						</select></td></tr>
					<tr>
						<td style="width: 200px;">Bidang Kerja</td>
						<td style="width: 300px;">
							<select style="width: 130px;" name="K_BIDANG_KERJA" class="required">
								<?php echo ShowOption(array( 'Array' => $array_bidang_kerja, 'ArrayID' => 'K_BIDANG_KERJA', 'ArrayTitle' => 'CONTENT' )); ?>
							</select>
							<span class="CntBidangKerja">
								<input type="text" style="width: 130px;" size="50" name="BIDANG_KERJA" />
							</span>
					</td></tr>
					<tr>
					...
					<tr class="non_fakultas">
						...
						<td style="width: 300px;"><select style="width: 85%;" name="K_JENJANG"><option>-</option></select></td></tr>
					<tr class="non_fakultas">
						<td style="width: 200px;">Fakultas</td>
						<td style="width: 300px;"><select style="width: 85%;" name="K_FAKULTAS"><option>-</option></select></td></tr>
					<tr class="non_fakultas">
						<td style="width: 200px;">Jurusan</td>
						<td style="width: 300px;"><select style="width: 85%;" name="K_JURUSAN"><option>-</option></select></td></tr>
					<tr class="non_fakultas">
						<td style="width: 200px;">Program Studi</td>
						<td style="width: 300px;"><select style="width: 85%;" name="K_PROG_STUDI"><option>-</option></select></td></tr>
					<tr>
						<td>Gaji Kontrak</td>
		...
		<div class="cnt-upload hidden">
			...
			<div id="form-upload" class="cnt_table_main">
				...
				<input type="hidden" name="ID_REQ_HONORER" />
				<input type="hidden" name="ID_RIWAYAT_HONORER" />
...
(function() {
	...
	var page = {
		...
		combo: {
			...
			bidang_kerja: function() {
				var value = $('[name="K_BIDANG_KERJA"]').val();
				if (value == '99') {
					$('.CntBidangKerja').show();
				} else {
					$('.CntBidangKerja').hide();
	...
	$('.record-new').click(function() {
		...
		$('.cnt-form [name="ID_RIWAYAT_HONORER"]').val('x');
		$('.cnt-form [name="JENIS_REQ_HONORER"]').val('I');
		$('.cnt-form [name="K_UNIT_KERJA"]').change();
		$('.cnt-form [name="K_BIDANG_KERJA"]').change();
	});
	$('.cnt-grid .btn-edit').click(function() {
		...
		$('.cnt-form [name="JENIS_REQ_HONORER"]').val(record.JENIS_REQ_HONORER);
		$('.cnt-form [name="ID_RIWAYAT_HONORER"]').val((record.ID_RIWAYAT_HONORER == null) ? 'x' : record.ID_RIWAYAT_HONORER);
		$('.cnt-form [name="ID_REQ_HONORER"]').val((record.ID_REQ_HONORER == null) ? 'x' : record.ID_REQ_HONORER);
		...
		$('.cnt-form [name="GAJI"]').val(record.GAJI);
		$('.cnt-form [name="KETERANGAN"]').val(record.KETERANGAN);
		$('.cnt-form [name="BIDANG_KERJA"]').val(record.BIDANG_KERJA);
		...
		if (action_form == 'update_request') {
			if (action == 'update_valid') {
				$('.cnt-form [name="JENIS_REQ_HONORER"]').val('U');
			} else {
				$('.cnt-form [name="JENIS_REQ_HONORER"]').val(record.JENIS_REQ_HONORER);
			}
	...
	$('.cnt-grid .btn-upload').click(function() {
		...
		// add param
		if (record.ID_REQ_HONORER != null)
			$('#form-upload [name="ID_REQ_HONORER"]').val(record.ID_REQ_HONORER);
		if (record.ID_RIWAYAT_HONORER != null)
			$('#form-upload [name="ID_RIWAYAT_HONORER"]').val(record.ID_RIWAYAT_HONORER);
	...
	$('.cnt-form [name="K_BIDANG_KERJA"]').change(function() {
