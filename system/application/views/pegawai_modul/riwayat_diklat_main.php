			...
			<?php if (count($array_pegawai_diklat) > 0) { ?>
				...
				<div class="cnt_table_main record-valid"><table style="width: 1300px;">
					...
					<tr>
						...
						<td class="body"><?php echo (@$row['JML_FILE'] == 0) ? '-' : 'Cek'; ?></td></tr>
					<?php } ?>
				</table></div>
			<?php } ?>
...
//		$('.cnt-form .btn-submit').show();
	...
	$('.cnt-grid .btn-edit').click(function() {
		...
		/*
		if (! page.data.is_user_fakultas && action == 'update_valid') {
			$('.cnt-form .btn-submit').show();
		} else if (page.data.is_user_fakultas && action == 'update_request') {
			$('.cnt-form .btn-submit').show();
		} else {
			$('.cnt-form .btn-submit').hide();
		}
		/*	*/
	});
