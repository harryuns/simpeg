...
class LLaporan_Ekd {
    ...
    function GetArray($Data) {
		...
		$Data['K_STATUS_KERJA'] = (isset($Data['K_STATUS_KERJA'])) ? $Data['K_STATUS_KERJA'] : 'x';
		
        $RawQuery = "CALL EKD.EKDLAPORANEVLS('".$Fakultas."', '".$Data['TAHUN']."', '".$Data['KESIMPULAN']."', '".$Data['K_STATUS_KERJA']."')";
    ...
    function GetContentHtml($Data, $Report) {
        ...
        if (!empty($Content)) {
			$LinkCetakHtml = HOST.'/index.php/LaporanEkd/Cetak/'.$Data['K_FAKULTAS'].'/'.$Data['TAHUN'].'/html/'.$Data['KESIMPULAN'].'/'.$Data['K_STATUS_KERJA'];
	...
	function SyncSimulasiRecord($Row) {
		if (!isset($Row['ALASAN']) || empty($Row['ALASAN'])) {
			return $Row;
		}
