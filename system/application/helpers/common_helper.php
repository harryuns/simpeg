...
if (! function_exists('StripArray')) {
	function StripArray($Array, $FieldName = array()) {
		...
		foreach($Array as $Key => $Element) {
			if (in_array($Key, $FieldName) && in_array($Element, array('0', '0000-00-00', '0001-01-01', '0000-00-00 00:00:00'))) {
...
if (! function_exists('upload_simpeg')) {
	function upload_simpeg($InputName, $path_dir = 'User', $Param = array()) {
		$Param['max_size'] = (isset($Param['max_size'])) ? $Param['max_size'] : 5000000;
		$Param['allow_ext'] = (isset($Param['allow_ext'])) ? $Param['allow_ext'] : array('jpg', 'jpeg', 'gif', 'png', 'bmp', 'doc', 'docx', 'pdf');
		...
		if (isset($_FILES[$InputName]) && is_array($_FILES[$InputName]) && is_array($_FILES[$InputName]['name'])) {
			...
			} else if (! in_array($Extention, $Param['allow_ext'])) {
				$ArrayResult['Message'] = 'Hanya file bertipe '.implode(', ', $Param['allow_ext']).' yang dapat di upload.';
			} else if ($_FILES[$InputName]['error'] == '0') {
				$DirYear = date("Y");
				$DirMonth = date("m");
				$DirDay = date("d");

				@mkdir($path_dir.'/'.$DirYear);
				@mkdir($path_dir.'/'.$DirYear.'/'.$DirMonth);
				@mkdir($path_dir.'/'.$DirYear.'/'.$DirMonth.'/'.$DirDay);

				$FileName = date("Ymd_His").'_'.rand(1000,9999).'.'.$Extention;
				$FileDirectory = $path_dir;
...
if (! function_exists('ftp_move_file')) {
	/*
	$p['file_path'] = $this->config->item('base_path').'/images/_temp/'.$param['FILENAME'];
	$p['ftp_upload_path'] = $this->config->item('base_path').'/images/upload';
	/*	*/
    function ftp_move_file($p) {
        $result = array('message' => '', 'status' => true);
		$p['create_dir'] = (isset($p['create_dir'])) ? $p['create_dir'] : 1;
        
		if (file_exists($p['file_path'])) {
			$ftp_conn = ftp_connect(SFTP_HOST);
			$ftp_resource = ftp_login($ftp_conn, SFTP_USER, SFTP_PASS);
			ftp_pasv($ftp_conn, (bool)true);
			...
			if ($p['create_dir'] == 1) {
				...
				foreach ($DirFile as $Directory) {
					$date_dir .= $Directory . '/';
					$p['ftp_upload_path'] .= '/' . $Directory;
					@ftp_mkdir($ftp_conn, $p['ftp_upload_path']);
				}
			}
			
			ftp_chdir($ftp_conn, $p['ftp_upload_path']);
			$file_desc = $p['ftp_upload_path'].'/'.basename($p['file_path']);
			$ftp_result = ftp_put($ftp_conn, $file_desc, $p['file_path'], FTP_BINARY);
        }
...
if (! function_exists('get_link_pegawai')) {
	function get_link_pegawai($value) {
		$result = RestoreLink($value);
		
		return $result;
	}
}

if (! function_exists('select_jenis_request')) {
	function select_jenis_request() {
		$array = array(
			'ArrayID' => 'id',
			'ArrayTitle' => 'nama',
			'Array' => array(
				array( 'id' => 'I', 'nama' => 'Tambah' ),
				array( 'id' => 'U', 'nama' => 'Ubah' ),
				array( 'id' => 'D', 'nama' => 'Hapus' )
			)
		);
		
		$result = '<select style="width: 150px;" name="JENIS_REQUEST">'.ShowOption($array).'</select>';
